...
func (r *PrivateFileRepository) Create(file entity.File) {
	...
	t := r.db.MustBegin()
	t.MustExec(query, file.Uuid, file.Path, file.Access, file.OwnerGuid, file.OwnerType, file.CreatedAt.String())
	t.Commit()

	//return t.Commit()
}

func (r *PrivateFileRepository) FindByUuid(uuid string) (*entity.File, error) {
	file := new(entity.File)

	query := fmt.Sprintf("SELECT * FROM %s WHERE access = $1 AND uuid = $2", r.table)
	err := r.db.Get(&file, query, "private", uuid)
...
func (r *PrivateFileRepository) DeleteByUuid(uuid string) error {
	...
	_, err := r.db.Exec(query, "private", uuid)
